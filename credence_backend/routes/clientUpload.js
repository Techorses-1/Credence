...
router.post("/upload", auth, upload.array("files"), async (req, res) => {
    try {
        ...
        // ========== START: DUPLICATE FILENAME VALIDATION (ADDED) ==========
        // Build set of existing filenames in this category
        ...
        // Check for duplicates within the new upload batch AND against existing files
        ...
        for (const file of req.files) {
            ...
            // Check for duplicate within the same upload batch
        ...
        // Remove duplicates from the array for cleaner error message
        const uniqueDuplicates = [...new Set(duplicateFiles)];

        if (uniqueDuplicates.length > 0) {
            return res.status(400).json({
                message: `❌ Cannot upload. Duplicate file(s): ${uniqueDuplicates.join(', ')}. Please remove duplicates and try again.`
            });
        }
        // ========== END: DUPLICATE FILENAME VALIDATION (ADDED) ==========
