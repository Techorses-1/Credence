...
/* ===============================
   GET PAYMENT STATUS FOR A MONTH - UPDATED FOR 3 STATUSES
================================ */
router.get("/clients/:clientId/payment-status", auth, async (req, res) => {
  try {
    ...
    // Handle backward compatibility - if paymentStatus is boolean, convert on the fly
    let paymentStatusValue = monthData.paymentStatus;
    if (typeof paymentStatusValue === 'boolean') {
      paymentStatusValue = paymentStatusValue === true ? 'paid' : 'pending';
    }
...
/* ===============================
   UPDATE PAYMENT STATUS FOR A MONTH - UPDATED FOR 3 STATUSES
================================ */
router.post("/clients/:clientId/payment-status", auth, async (req, res) => {
  try {
    ...
    // ✅ UPDATED: Validate status is one of the 3 allowed values
    if (!year || !month || !status) {
      return res.status(400).json({
        success: false,
        message: "Year, month, and status are required"
      });
    }

    const allowedStatuses = ['paid', 'pending', 'not_credited'];
    if (!allowedStatuses.includes(status)) {
      return res.status(400).json({
        success: false,
        message: "Status must be one of: paid, pending, not_credited"
      });
    }
    ...
    // Handle backward compatibility - if paymentStatus is boolean, convert on the fly
    let previousStatus = monthData.paymentStatus;
    if (typeof previousStatus === 'boolean') {
      previousStatus = previousStatus === true ? 'paid' : 'pending';
    }

    if (previousStatus === status) {
      return res.json({
        success: true,
        message: `Payment status already ${getStatusDisplayName(status)}`,
    ...
    monthData.paymentHistory.push({
      ...
      notes: notes || `Changed from ${getStatusDisplayName(previousStatus)} to ${getStatusDisplayName(status)}`
    });
    ...
    const actionDetails = `Marked payment as ${getStatusDisplayName(status)} for ${client.name} - ${month}/${year}`;
    ...
    res.json({
      success: true,
      message: `Payment status updated to ${getStatusDisplayName(status)} successfully`,
      paymentStatus: status,
      paymentUpdatedAt: monthData.paymentUpdatedAt,
      paymentUpdatedBy: monthData.paymentUpdatedBy,
      paymentUpdatedByName: monthData.paymentUpdatedByName,
...
/* ===============================
   GET PAYMENT HISTORY FOR A MONTH - UPDATED FOR 3 STATUSES
================================ */
router.get("/clients/:clientId/payment-history", auth, async (req, res) => {
  try {
    ...
    // Handle backward compatibility for currentStatus
    let currentStatus = monthData.paymentStatus;
    if (typeof currentStatus === 'boolean') {
      currentStatus = currentStatus === true ? 'paid' : 'pending';
    }

    // Handle backward compatibility for history entries
    let paymentHistory = monthData.paymentHistory || [];
    paymentHistory = paymentHistory.map(entry => {
      if (typeof entry.status === 'boolean') {
        return {
          ...entry.toObject(),
          status: entry.status === true ? 'paid' : 'pending'
        };
      }
      return entry;
    });
