...
/* ===============================
   OPTIMIZED: GET NOTES FOR SPECIFIC ASSIGNED CLIENT
   Uses batch-loaded data to reduce DB queries
================================ */
router.get("/client/:clientId/notes", async (req, res) => {
  try {
    ...
    logToConsole("INFO", "EMPLOYEE_GET_CLIENT_NOTES_OPTIMIZED", { clientId, year, month, ip: req.ip });
    ...
    // ===== OPTIMIZATION: Get client AND monthly data in PARALLEL =====
    const [client, monthlyDataDoc] = await Promise.all([
      Client.findOne({ clientId }).lean(),
      ClientMonthlyData.findOne({ clientId }).lean()
    ]);
    ...
    // ===== OPTIMIZATION: Build month data map for O(1) lookup =====
    const monthDataMap = new Map();
    if (monthlyDataDoc && monthlyDataDoc.months && Array.isArray(monthlyDataDoc.months)) {
      for (const monthData of monthlyDataDoc.months) {
        const key = `${monthData.year}-${monthData.month}`;
        monthDataMap.set(key, monthData);
      }
    }

    // ===== OPTIMIZATION: Process OLD documents in memory without extra DB calls =====
    if (client.documents) {
      let documentsObj = {};
      if (client.documents instanceof Map) {
        for (const [yearKey, yearMap] of client.documents.entries()) {
      ...
      // Merge OLD documents into map (if not already in NEW collection)
      for (const yearKey in documentsObj) {
        const year = parseInt(yearKey);
        const yearData = documentsObj[yearKey];
        if (!yearData || typeof yearData !== 'object') continue;
        ...
        for (const monthKey in yearData) {
          ...
          if (!monthDataMap.has(key)) {
            monthDataMap.set(key, yearData[monthKey]);
          }
        }
      }
    }

    // ===== OPTIMIZATION: Extract notes using map (no extra DB calls) =====
    const extractNotesFromMap = (clientDoc, employeeId, monthDataMap, assignedMonthKeys) => {
      const notes = [];
      const clientIdVal = clientDoc.clientId;
      const clientName = clientDoc.name || `${clientDoc.firstName} ${clientDoc.lastName}` || 'Unknown Client';
    ...
    let notes = extractNotesFromMap(client, employee.employeeId, monthDataMap, assignedMonthKeys);
    ...
    // Group by month (same logic as original)
    const notesByMonth = {};
    notes.forEach(note => {
    ...
    const assignedMonths = Array.from(assignedMonthKeys)
      .map(key => {
        const [yearVal, monthVal] = key.split('-');
        return {
          year: parseInt(yearVal),
          month: parseInt(monthVal),
          monthName: new Date(yearVal, monthVal - 1).toLocaleString('default', { month: 'long', timeZone: "Europe/Helsinki" })
        };
      })
      .sort((a, b) => {
        if (a.year !== b.year) return b.year - a.year;
...
/* ===============================
   OPTIMIZED: MARK NOTES AS VIEWED BY EMPLOYEE
   Uses bulk operations and reduces DB round trips
================================ */
router.post("/mark-as-viewed", async (req, res) => {
  try {
    const { clientId, noteIds, filter, markAll = false } = req.body;

    logToConsole("INFO", "EMPLOYEE_MARK_NOTES_AS_VIEWED_OPTIMIZED", { clientId, noteIdsCount: noteIds?.length || 0, markAll, filter, ip: req.ip });
    ...
    // ===== OPTIMIZATION: Get client AND monthly data in PARALLEL (single DB trip each) =====
    const [client, monthlyDataDoc] = await Promise.all([
      Client.findOne({ clientId }).lean(),
      ClientMonthlyData.findOne({ clientId }).lean()
    ]);
    ...
    // ===== OPTIMIZATION: Extract notes using map (no extra DB calls) =====
    const extractNotesForMarking = (clientDoc, employeeId, monthDataMap, assignedMonthKeys) => {
      const notes = [];
      const clientIdVal = clientDoc.clientId;

      const addNoteForMarking = (noteObj, context) => {
        if (!noteObj || !noteObj.note) return;
      ...
      const processMonthData = (monthData, year, month) => {
        ...
        if (monthData.monthNotes && Array.isArray(monthData.monthNotes)) {
          monthData.monthNotes.forEach(note => {
            addNoteForMarking(note, { contextType: 'month', year, month, noteLevel: 'month' });
          });
        }
        ...
        ['sales', 'purchase', 'bank'].forEach(categoryType => {
          if (monthData[categoryType] && typeof monthData[categoryType] === 'object') {
            ...
            if (category.categoryNotes && Array.isArray(category.categoryNotes)) {
              category.categoryNotes.forEach(note => {
                addNoteForMarking(note, { contextType: 'category', year, month, categoryType, noteLevel: 'category' });
              });
            }
            ...
            if (category.files && Array.isArray(category.files)) {
              category.files.forEach(file => {
                if (file && file.notes && Array.isArray(file.notes)) {
                  file.notes.forEach(note => {
                    addNoteForMarking(note, { contextType: 'file', year, month, categoryType, fileName: file.fileName, noteLevel: 'file' });
                  });
                }
              });
            }
        ...
        if (monthData.other && Array.isArray(monthData.other)) {
          monthData.other.forEach(otherCategory => {
            if (otherCategory && otherCategory.document && typeof otherCategory.document === 'object') {
              ...
              if (otherCategory.document.categoryNotes && Array.isArray(otherCategory.document.categoryNotes)) {
                otherCategory.document.categoryNotes.forEach(note => {
                  addNoteForMarking(note, { contextType: 'category', year, month, categoryType, categoryName, noteLevel: 'category' });
                });
              }
              ...
              if (otherCategory.document.files && Array.isArray(otherCategory.document.files)) {
                otherCategory.document.files.forEach(file => {
                  if (file && file.notes && Array.isArray(file.notes)) {
                    file.notes.forEach(note => {
                      addNoteForMarking(note, { contextType: 'file', year, month, categoryType, categoryName, fileName: file.fileName, noteLevel: 'file' });
                    });
                  }
                });
              }
    ...
    let notesToMark = extractNotesForMarking(client, employee.employeeId, monthDataMap, assignedMonthKeys);

    // Apply filters (same logic as original)
    if (filter) {
      const { year: filterYear, month: filterMonth } = filter;
      if (filterYear) {
    ...
    // ===== OPTIMIZATION: Use direct update queries instead of load-modify-save =====
    let markedCount = 0;
    const now = new Date();
    const viewEntry = {
    ...
    // Helper to create update paths for nested notes
    const buildUpdateOperations = (monthData, year, month, unreadNotesToMark, employeeId) => {
      const updates = [];
      
      const findMatchingNote = (note, context) => {
        return unreadNotesToMark.find(unreadNote =>
          unreadNote.note === note.note &&
          unreadNote.addedBy === note.addedBy &&
          new Date(unreadNote.addedAt).getTime() === new Date(note.addedAt).getTime() &&
          year === unreadNote.year &&
      ...
      if (monthData.monthNotes && Array.isArray(monthData.monthNotes)) {
        monthData.monthNotes.forEach((note, index) => {
          if (findMatchingNote(note, { noteLevel: 'month' }) && !note.isViewedByEmployee) {
            updates.push({
              path: `months.$[monthElem].monthNotes.${index}.isViewedByEmployee`,
              value: true
            });
            updates.push({
              path: `months.$[monthElem].monthNotes.${index}.viewedBy`,
              value: [...(note.viewedBy || []), viewEntry]
            });
          }
        });
      }
      ...
      ['sales', 'purchase', 'bank'].forEach(categoryType => {
        if (monthData[categoryType] && monthData[categoryType].categoryNotes) {
          monthData[categoryType].categoryNotes.forEach((note, index) => {
            if (findMatchingNote(note, { noteLevel: 'category', categoryType }) && !note.isViewedByEmployee) {
              updates.push({
                path: `months.$[monthElem].${categoryType}.categoryNotes.${index}.isViewedByEmployee`,
                value: true
              });
              updates.push({
                path: `months.$[monthElem].${categoryType}.categoryNotes.${index}.viewedBy`,
                value: [...(note.viewedBy || []), viewEntry]
              });
            }
          });
        ...
        if (monthData[categoryType] && monthData[categoryType].files) {
          monthData[categoryType].files.forEach((file, fileIndex) => {
            if (file.notes) {
              file.notes.forEach((note, noteIndex) => {
                if (findMatchingNote(note, { noteLevel: 'file', categoryType, fileName: file.fileName }) && !note.isViewedByEmployee) {
                  updates.push({
                    path: `months.$[monthElem].${categoryType}.files.${fileIndex}.notes.${noteIndex}.isViewedByEmployee`,
                    value: true
                  });
                  updates.push({
                    path: `months.$[monthElem].${categoryType}.files.${fileIndex}.notes.${noteIndex}.viewedBy`,
                    value: [...(note.viewedBy || []), viewEntry]
                  });
                }
              });
      ...
      if (monthData.other && Array.isArray(monthData.other)) {
        monthData.other.forEach((otherCat, otherIndex) => {
          if (otherCat.document) {
            // Category notes
            if (otherCat.document.categoryNotes) {
              otherCat.document.categoryNotes.forEach((note, noteIndex) => {
                if (findMatchingNote(note, { noteLevel: 'category', categoryType: 'other', categoryName: otherCat.categoryName }) && !note.isViewedByEmployee) {
                  updates.push({
                    path: `months.$[monthElem].other.${otherIndex}.document.categoryNotes.${noteIndex}.isViewedByEmployee`,
                    value: true
                  });
                  updates.push({
                    path: `months.$[monthElem].other.${otherIndex}.document.categoryNotes.${noteIndex}.viewedBy`,
                    value: [...(note.viewedBy || []), viewEntry]
                  });
                }
              });
            ...
            if (otherCat.document.files) {
              otherCat.document.files.forEach((file, fileIndex) => {
                if (file.notes) {
                  file.notes.forEach((note, noteIndex) => {
                    if (findMatchingNote(note, { noteLevel: 'file', categoryType: 'other', categoryName: otherCat.categoryName, fileName: file.fileName }) && !note.isViewedByEmployee) {
                      updates.push({
                        path: `months.$[monthElem].other.${otherIndex}.document.files.${fileIndex}.notes.${noteIndex}.isViewedByEmployee`,
                        value: true
                      });
                      updates.push({
                        path: `months.$[monthElem].other.${otherIndex}.document.files.${fileIndex}.notes.${noteIndex}.viewedBy`,
                        value: [...(note.viewedBy || []), viewEntry]
                      });
                    }
                  });
    ...
    // ===== OPTIMIZATION: Update NEW collection using $set with array filters =====
    if (monthlyDataDoc && monthlyDataDoc.months) {
      const bulkUpdates = [];
      
      for (let i = 0; i < monthlyDataDoc.months.length; i++) {
        const monthData = monthlyDataDoc.months[i];
        const monthKey = `${monthData.year}-${monthData.month}`;
        
        if (assignedMonthKeys.has(monthKey)) {
          const updates = buildUpdateOperations(monthData, monthData.year, monthData.month, unreadNotesToMark, employee.employeeId);
          
          for (const update of updates) {
            const updateObj = { $set: { [update.path]: update.value } };
            const arrayFilters = [{ "monthElem.year": monthData.year, "monthElem.month": monthData.month }];
      ...
      if (bulkUpdates.length > 0) {
        const result = await ClientMonthlyData.bulkWrite(bulkUpdates);
        markedCount += bulkUpdates.length;
        logToConsole("DEBUG", "NEW_COLLECTION_BULK_UPDATE", { matchedCount: result.matchedCount, modifiedCount: result.modifiedCount });
      }
    }

    // ===== OPTIMIZATION: Update OLD collection using direct update =====
    if (client.documents) {
      const oldDocUpdates = [];
      
      for (const monthKey of assignedMonthKeys) {
        const [year, month] = monthKey.split('-');
        const monthData = monthDataMap.get(monthKey);
        
        if (monthData) {
          const updates = buildUpdateOperations(monthData, parseInt(year), parseInt(month), unreadNotesToMark, employee.employeeId);
          
          for (const update of updates) {
            // Convert path for OLD document structure
            let oldPath = update.path.replace('months.$[monthElem].', `documents.${year}.${month}.`);
            oldPath = oldPath.replace(/\./g, '.');
            
            oldDocUpdates.push({
              updateOne: {
                filter: { clientId: clientId },
                update: { $set: { [oldPath]: update.value } }
              }
            });
          }
        }
      ...
      if (oldDocUpdates.length > 0) {
        const result = await Client.bulkWrite(oldDocUpdates);
        markedCount += oldDocUpdates.length;
        logToConsole("DEBUG", "OLD_COLLECTION_BULK_UPDATE", { matchedCount: result.matchedCount, modifiedCount: result.modifiedCount });
      }
    }

    // ===== OPTIMIZATION: Get updated unread count using map (no extra DB query) =====
    const getUpdatedUnreadCount = (notesList, assignedMonthKeys) => {
      return notesList.filter(n => !n.isViewedByEmployee && assignedMonthKeys.has(`${n.year}-${n.month}`)).length;
    };

    // Re-extract notes to get updated status (using same map data)
    const updatedNotesList = extractNotesForMarking(client, employee.employeeId, monthDataMap, assignedMonthKeys);
    const updatedUnread = getUpdatedUnreadCount(updatedNotesList, assignedMonthKeys);

    logToConsole("SUCCESS", "EMPLOYEE_NOTES_MARKED_AS_VIEWED_OPTIMIZED", {
      employeeId: employee.employeeId,
      clientId,
      markedCount,
